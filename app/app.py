...
# Repo root = the parent of this file's folder (this file lives in app/,
# models/ and data/ are siblings of app/ at the repo root). Anchoring
# paths this way makes the app work regardless of what directory
# Streamlit Cloud happens to launch from.
...
@st.cache_data
def load_data():
    df = pd.read_csv(BASE_DIR / "data" / "scored_loan_tape.csv",
...
# ---------------------------------------------------------------------
    ...
    with left:
        ...
        detail = {
            ...
            "Origination": str(loan["origination_date"].date()),
            "Maturity": str(loan["maturity_date"].date()),
...
# ---------------------------------------------------------------------
    ...
    st.dataframe(delta_df.sort_values("increase", ascending=False).head(10), width='stretch')
